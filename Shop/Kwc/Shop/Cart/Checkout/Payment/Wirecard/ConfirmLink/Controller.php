<?php
class Shop_Kwc_Shop_Cart_Checkout_Payment_Wirecard_ConfirmLink_Controller extends Zend_Controller_Action
{
    ...
    public function jsonConfirmOrderAction()
    {
        ...
        $order = Kwf_Model_Abstract::getInstance(Kwc_Abstract::getSetting($component->getParentByClass('Shop_Kwc_Shop_Cart_Component')->componentClass, 'childModel'))
            ->getReferencedModel('Order')->getCartOrder();
        if ($order && $component &&
            is_instance_of($component->componentClass, 'Shop_Kwc_Shop_Cart_Checkout_Payment_Wirecard_Component')
        ) {

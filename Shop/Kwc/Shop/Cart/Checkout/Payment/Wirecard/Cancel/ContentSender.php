<?php
class Shop_Kwc_Shop_Cart_Checkout_Payment_Wirecard_Cancel_ContentSender extends Kwf_Component_Abstract_ContentSender_Default
{
    public function sendContent($includeMaster)
    {
        $session = new Kwf_Session_Namespace('kwcShopCart');
        if ($session->wirecardCartId) {
            Shop_Kwc_Shop_Cart_Orders::setCartOrderId($session->wirecardCartId);
            ...
            $db->query(
                "UPDATE `Shop_Kwc_Shop_orders` SET `status` = 'cart' WHERE `id` = {$db->quote($order->id)} AND `status` = 'processing'"
            );

<?php
class Shop_Kwc_Shop_Cart_Orders extends Kwf_Model_Db
{
    protected $_table = 'Shop_Kwc_Shop_orders';
    protected $_rowClass = 'Shop_Kwc_Shop_Cart_Order';
    protected $_siblingModels = array('Shop_Kwc_Shop_Cart_Checkout_Model');
    protected $_dependentModels = array('Products'=>'Shop_Kwc_Shop_Cart_OrderProducts');
    ...
    public function getCartComponentClass()
    {
        ...
        if (!$ret = Kwf_Cache_SimpleStatic::fetch($cacheId)) {
            ...
            foreach (Kwc_Abstract::getComponentClasses() as $c) {
                if (is_instance_of($c, 'Shop_Kwc_Shop_Cart_Component')) {
                    $classes[] = $c;
                }
            }
            if (count($classes) != 1) {
                throw new Kwf_Exception("Not exactly one Shop_Kwc_Shop_Cart_Component found, set _cartComponentClass manually");
            }
            $ret = $classes[0];
    ...
    public final function getShopCartPlugins()
    {
        if (!isset($this->_chartPlugins)) {
            ...
            foreach ($plugins as $plugin) {
                if (is_instance_of($plugin, 'Shop_Kwc_Shop_Cart_Plugins_Interface')) {

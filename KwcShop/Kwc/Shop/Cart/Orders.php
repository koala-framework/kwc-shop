    ...
    public function getCartComponentClass()
    {
        ...
        if (!$ret = Kwf_Cache_SimpleStatic::fetch($cacheId)) {
            ...
            foreach (Kwc_Abstract::getComponentClasses() as $c) {
                if (is_instance_of($c, 'KwcShop_Kwc_Shop_Cart_Component')) {
                    $classes[] = $c;
                }
            }
            if (count($classes) != 1) {
                throw new Kwf_Exception("Not exactly one KwcShop_Kwc_Shop_Cart_Component found, set _cartComponentClass manually");
            }
            $ret = $classes[0];
    ...
    public final function getShopCartPlugins()
    {
        if (!isset($this->_chartPlugins)) {
            ...
            foreach ($plugins as $plugin) {
                if (is_instance_of($plugin, 'KwcShop_Kwc_Shop_Cart_Plugins_Interface')) {

<?php
class KwcShop_Kwc_Shop_Cart_Order extends Kwf_Model_Db_Row
    implements Kwc_Mail_Recipient_TitleInterface
{
    ...
    {
        parent::_afterSave();
        if (KwcShop_Kwc_Shop_Cart_Orders::getCartOrderId() == $this->id && $this->status != 'cart') {
    ...
    {
        $data = KwcShop_Kwc_Shop_VoucherProduct_AddToCart_OrderProductData::getInstance($orderProduct->add_component_class);
    ...
    public function getTotalAmount()
    {
        $ret = 0;
        foreach ($this->getChildRows('Products') as $op) {
            $data = KwcShop_Kwc_Shop_VoucherProduct_AddToCart_OrderProductData::getInstance($op->add_component_class);
    ...
    // if product is not available in sitetree anymore it is deleted (also called by KwcShop_Kwc_Shop_Cart_Component)
    private function _getProductsData(Kwf_Component_Data $subroot = null)
    {
        ...
        foreach ($items as $i) {
            $data = KwcShop_Kwc_Shop_VoucherProduct_AddToCart_OrderProductData::getInstance($i->add_component_class);
            $r = array(
                'id' => $i->id,
    ...
    protected function _getAdditionalSumRows($total)
    {
        ...
        if (isset($payments[$this->payment])) {
            $rows = KwcShop_Kwc_Shop_Cart_Checkout_Payment_Abstract_OrderData

<?php
class KwcShop_Kwc_Shop_Cart_Checkout_Payment_Wirecard_Cancel_ContentSender extends Kwf_Component_Abstract_ContentSender_Default
{
    public function sendContent($includeMaster)
    {
        $session = new Kwf_Session_Namespace('kwcShopCart');
        if ($session->wirecardCartId) {
            KwcShop_Kwc_Shop_Cart_Orders::setCartOrderId($session->wirecardCartId);

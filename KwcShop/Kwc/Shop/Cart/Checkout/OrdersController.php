<?php
class KwcShop_Kwc_Shop_Cart_Checkout_OrdersController_Payment extends Kwf_Data_Abstract
{
    private $_payments;
...
class KwcShop_Kwc_Shop_Cart_Checkout_OrdersController_SumAmount extends Kwf_Data_Abstract
{
    public function load($row, array $info = array())
    {
        $ret = 0;
        foreach ($row->getChildRows('Products') as $p) {
            $data = KwcShop_Kwc_Shop_AddToCartAbstract_OrderProductData::getInstance($p->add_component_class);
            $ret += $data->getAmount($p);
        }
        return $ret;
    }
}
class KwcShop_Kwc_Shop_Cart_Checkout_OrdersController_SumPrice extends Kwf_Data_Abstract
{
    public function load($row, array $info = array())
    {
        return $row->getTotal();
...
class KwcShop_Kwc_Shop_Cart_Checkout_OrdersController extends Kwf_Controller_Action_Auto_Grid
{
    ...
    protected function _initColumns()
    {
        ...
        $this->_columns->add(new Kwf_Grid_Column('sum_amount', trlKwf('Amt'), 30))
            ->setData(new KwcShop_Kwc_Shop_Cart_Checkout_OrdersController_SumAmount())
            ->setSortable(false);
        $this->_columns->add(new Kwf_Grid_Column('sum_price', trlKwf('Sum'), 50))
            ->setData(new KwcShop_Kwc_Shop_Cart_Checkout_OrdersController_SumPrice())
            ->setSortable(false)
            ->setRenderer('euroMoney');
        if (count($payments) > 1) {
            $this->_columns->add(new Kwf_Grid_Column('payment', trlKwf('Payment'), 80))
                ->setData(new KwcShop_Kwc_Shop_Cart_Checkout_OrdersController_Payment($payments))
                ->setSortable(false);
        }
